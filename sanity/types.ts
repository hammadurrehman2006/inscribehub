...
// Variable: Author_BLOGS_QUERY
// Query: *[_type == "blog" && defined(slug.current) && author._ref == $id] | order(_createdAt desc){    _id,    title,    slug,    _createdAt,author -> {_id, name, image, bio},views,description,category,image    }
export type Author_BLOGS_QUERYResult = Array<{
  _id: string;
  title: string | null;
  slug: Slug | null;
  _createdAt: string;
...
// Query: *[_type == "playlist" && slug.current == $slug][0]{  _id,  title,  slug,  select[]->{    _id,    _createdAt,    title,    slug,    author->{      _id,      name,      slug,      image,      bio    },    views,    description,    category,    image,    blog  }}
